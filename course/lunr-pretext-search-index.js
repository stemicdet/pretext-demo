...
},
{
  ...
  "type": "Section",
  "number": "",
  "title": "Week 7: Integration with Respect to a Measure (3A Plus 6.2 in A Radical Approach to Lebesgue’s Theory of Integration)",
  "body": " Week 7: Integration with Respect to a Measure (3A Plus 6.2 in A Radical Approach to Lebesgue's Theory of Integration)   Integration of Simple Functions  Now that we have developed the theory of measures, we have a way to assign a length to subsets of . Thus we have the tools to define integration in the sense of Lebesgue. We will build up the Lebesgue integral in stages. First we define the Lebesgue integral for simple functions, and then we extend this definition to non-negative functions. We can then generalize this to all functions by splitting them up into positive and negative parts. Recall that the main idea of Lebesgue integration is that we are going to partition the range, rather than the domain. It is convenient to first define integration on simple functions, since the range of a simple function is finite.   Lebesgue Integral of Simple Functions   Suppose is a measure space, are disjoint sets in , and . Let Then we define the Lebesgue integral of with respect to the measure over the measurable set to be     In other words, for each element in the range, we multiply it by the size of the pre-image and take the sum over all elements in the range.  Example: Consider the function on defined by    is a simple function because it can be rewritten as   Let denote Lebesgue measure. It thus follows that   Thus the Lebesgue integral gives the result that we would expect from Calculus and geometry.  Recall that Dirichlet's function is not Riemann integrable. However, since it is the characteristic function of the rationals, it is Lebesgue integrable and    Properties of the Lebesgue Integral for Simple Functions   Let be a measure space, and be simple functions, and , where and are disjoint measurable sets. The following properties hold             If for all , then .            The last statement is a generalization of the familiar property .   We start by writing and as where the 's are distinct, the 's are distinct, the 's are pairwise disjoint, the 's are pairwise disjoint, and .  To prove the first proposition, it follows from the definition of the Lebesgue integral that   To prove the second proposition, note that is a simple function since   Hence Thus the second proposition is proved.  To prove the third proposition we will rewrite our functions as Since on , it follows that for each pair, for which .  Hence,   Finally, to prove the last proposition we use the fact that measures are countably additive.    The Monotone Convergence Theorem (Also called the Beppo Levi Theorem) is one of the key results in the theory of Lebesgue Integration. We first prove it for simple functions.   Monotone Convergence Theorem for Simple Functions   Let be a monotonically increasing sequence of non-negative simple functions. If there is a finite for which for all , then converges to a finite valued function almost everywhere.     Almost Everywhere   A property is said to occur almost everywhere if the set where it does not occur has measure zero.    In probability, an event is said to occur almost surely if it happens with probability . This is the same as saying that the probability that the event does not occur is .  Example: Suppose you choose a real number at random. Since the set of rational numbers is a set with measure , the probability that the chosen number is rational is . Thus the event that the chosen number is irrational occurs almost surely.   Recall the Monotone Convergence Theorem which states that a bounded monotonic sequence converges. This means that we can prove that converges almost everywhere by showing that the set of points in for which is unbounded is a set of measure zero.  Choose and let   Then since is non-negative,   Hence . Since is increasing, we have that .  Let denote the set of for which is unbounded. Then is contained in the union of the 's. Thus it follows from monotonicity and the theorem about measure of an increasing union that Since the above inequality is true for all , it follows that . Hence is bounded almost everywhere and thus converges to a finite valued function almost everywhere.     Integration of Non-negative Functions  We now extend the definition of Lebesgue integration to non-negative functions. The main idea is that every bounded non-negative function can be approximated from below by a sequence of simple functions.   Lebesgue Integral of Non-negative Measurable Functions   Let be a measure space. Given a non-negative measurable function and a measurable set , we define where the supremum is taken over all simple functions for which for all .    Example: Let . We'll evaluate using the definition of Lebesgue integration. Choose and divide into equally spaced intervals of width . We define a simple function by setting it equal to the left endpoint of each interval. That is, Then is a simple function and on .  It follows that   Taking the supremum over all gives .  In the Lebesgue integral the supremum must be taken over all simple functions with , not just the 's. However, if is a simple function with , then there exists such that and thus by the properties of the integrals of simple functions . Thus we are justified in only taking the supremum over the 's and can conclude that    Integral over a Set of Measure Zero   Let be a measure space.    If is a measurable function and , then .    If , where and are disjoint measurable sets, then .    If almost everywhere, then        For part (a) it follows from the definition of the Lebesgue integral for simple functions, that every simple function on has integral . Thus the supremum over all simple functions is also and .  For the proof of (b), we showed that for simple functions we have Taking the supremum over all simple functions with thus gives that   To prove (c), let denote the subset of such that and let . Then and     Monotone Convergence Theorem   Let be a measure space and let be a monotonically increasing sequence of non-negative functions. If there is a finite for which for all , then converges to a finite-valued function almost everywhere, is integrable, and      We will first prove that converges almost everywhere, then we will show that and lastly we will show that   From the monotone convergent Theorem we know that for a given , converges provided that the sequence is bounded. Thus we want to show that the set of where is unbounded has measure zero. We know that each function is the limit of an increasing sequence of simple functions . Define a new sequence of simple functions by   Since  is an increasing sequence.  Since , the integrals are bounded. If is a point for which is unbounded, then for any , we can find an for which . Since converges to , we can find a for which .  Hence and thus the sequence is also unbounded.  By the Monotone Convergence Theorem for Simple Functions , the set of for which is unbounded has measure zero. Thus we are free to set on . We now have that is bounded almost everywhere and thus converges to a finite valued function almost everywhere, which is the first part of the proof.  For the next part suppose that and are non-negative measurable functions with for all . Then the set of simple functions that are less than or equal to is a subset of the set of simple functions that are less than or equal to and hence   If except possibly for where , then   We have shown that functional inequalities that hold almost everywhere imply integral inequalities that hold on all . Since almost everywhere, it follows that for all  and therefore,   To finish the proof of the Monotone Convergence Theorem we need to show the reverse inequality holds as well. To show the reverse inequality let be a non-negative simple function that is less than or equal to . That is, where the are disjoint sets with .  Choose with and define the sets   Since is an increasing sequence it follows that is an increasing sequence of sets with . Hence   Now choose . Then for each , we can find such that whenever ,   Let . Then for ,   Hence,   Since this is true for every and with , it follows that   Thus every simple function has an integral bounded above by for all sufficiently large. Hence and thus which completes the proof.    Integration is Homogeneous   Suppose is a measure space and is a non-negative function. If , then      Note that is a simple function with if and only if is a simple function with . Hence      Integration of Real Valued Functions  We first note that every real valued function can be written as a difference of non-negative functions.   and   Suppose is a function. Define functions and by     Note that and .   Integral of a Real Valued Function   Suppose is a measure space and is an -measurable function such that at least one of or is finite. The integral of with respect to denoted by , is defined by     The geometrical interpretation of this definition is that we are subtracting the integral of the negative part of the function from the integral of the positive part, much like how in Calculus we found the value of the integral by subtracting the area of the function beneath the axis from the area of the function above the axis.  Example: Suppose that Let denote Lebesgue measure. Find   Example: Suppose that Then is not defined because .  We can now verify that the familiar properties of integration hold in general for the Riemann integral.   Integration is Homogeneous   Suppose is a measure space and is a measurable function such that is defined. If , then      First suppose . Then and . Hence   If , then and thus and . Hence     Additivity of Integration   Suppose is a measure space and are -measurable functions such that and . Then      We first observe that . Hence   Since both sides are non-negative functions we can integrate both sides of the above equation with respect to to get Rearranging the above equation gives   We note that the left side is not of the form since and . Thus we can simplify the above equation to obtain which is what we wanted to prove.    Integration is Order Preserving   Suppose is a measure space and are -measurable functions such that and are defined. Suppose also that for all . Then .     If or then the conclusion is always true, so suppose that and . It follows from additivity that   The right hand side is non-negative since for all . Moreover, the left hand side is finite due to our assumptions. Hence     Absolute Value of Integral than Integral of Absolute Value   Suppose is a function such that is defined. Then      Because is defined and is an -measureable function at least one of and is finite. Hence, which is what we want to show.   In probability the expected value is a generalization of the weighted average. In the case that there are infinitely many possible outcomes, the expected value is defined using the Lebesgue integral.   Expected Value   Suppose is a real valued random variable on a probability space . Then the expected value of is denoted is defined as the Lebesgue integral     If has only a finite number of possible outcomes, then this definition reduces to the weighted average of the outcomes.   "
},
{
  "id": "def-simple-function-integral",
  "level": "2",
  "url": "notes-week-07.html#def-simple-function-integral",
  "type": "Definition",
  "number": "69",
  "title": "Lebesgue Integral of Simple Functions.",
  "body": " Lebesgue Integral of Simple Functions   Suppose is a measure space, are disjoint sets in , and . Let Then we define the Lebesgue integral of with respect to the measure over the measurable set to be    "
},
{
  "id": "thm-properties-integral-simple-functions",
  "level": "2",
  "url": "notes-week-07.html#thm-properties-integral-simple-functions",
  "type": "Theorem",
  "number": "70",
  "title": "Properties of the Lebesgue Integral for Simple Functions.",
  "body": " Properties of the Lebesgue Integral for Simple Functions   Let be a measure space, and be simple functions, and , where and are disjoint measurable sets. The following properties hold             If for all , then .           "
},
{
  "id": "subsec-Integration-of-Simple-Functions-12",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Simple-Functions-12",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " We start by writing and as where the 's are distinct, the 's are distinct, the 's are pairwise disjoint, the 's are pairwise disjoint, and .  To prove the first proposition, it follows from the definition of the Lebesgue integral that   To prove the second proposition, note that is a simple function since   Hence Thus the second proposition is proved.  To prove the third proposition we will rewrite our functions as Since on , it follows that for each pair, for which .  Hence,   Finally, to prove the last proposition we use the fact that measures are countably additive.   "
},
{
  "id": "thm-monotone-convergence-simple",
  "level": "2",
  "url": "notes-week-07.html#thm-monotone-convergence-simple",
  "type": "Theorem",
  "number": "71",
  "title": "Monotone Convergence Theorem for Simple Functions.",
  "body": " Monotone Convergence Theorem for Simple Functions   Let be a monotonically increasing sequence of non-negative simple functions. If there is a finite for which for all , then converges to a finite valued function almost everywhere.   "
},
{
  "id": "def-almost-everywhere",
  "level": "2",
  "url": "notes-week-07.html#def-almost-everywhere",
  "type": "Definition",
  "number": "72",
  "title": "Almost Everywhere.",
  "body": " Almost Everywhere   A property is said to occur almost everywhere if the set where it does not occur has measure zero.   "
},
{
  "id": "subsec-Integration-of-Simple-Functions-16",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Simple-Functions-16",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "almost surely "
...
{
  "id": "subsec-Integration-of-Simple-Functions-18",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Simple-Functions-18",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " Recall the Monotone Convergence Theorem which states that a bounded monotonic sequence converges. This means that we can prove that converges almost everywhere by showing that the set of points in for which is unbounded is a set of measure zero.  Choose and let   Then since is non-negative,   Hence . Since is increasing, we have that .  Let denote the set of for which is unbounded. Then is contained in the union of the 's. Thus it follows from monotonicity and the theorem about measure of an increasing union that Since the above inequality is true for all , it follows that . Hence is bounded almost everywhere and thus converges to a finite valued function almost everywhere.  "
},
{
  "id": "def-integral-non-negative",
  "level": "2",
  "url": "notes-week-07.html#def-integral-non-negative",
  "type": "Definition",
  "number": "73",
  "title": "Lebesgue Integral of Non-negative Measurable Functions.",
  "body": " Lebesgue Integral of Non-negative Measurable Functions   Let be a measure space. Given a non-negative measurable function and a measurable set , we define where the supremum is taken over all simple functions for which for all .   "
},
{
  "id": "thm-integral-over-set-of-measure-0",
  "level": "2",
  "url": "notes-week-07.html#thm-integral-over-set-of-measure-0",
  "type": "Theorem",
  "number": "74",
  "title": "Integral over a Set of Measure Zero.",
  "body": " Integral over a Set of Measure Zero   Let be a measure space.    If is a measurable function and , then .    If , where and are disjoint measurable sets, then .    If almost everywhere, then      "
},
{
  "id": "subsec-Integration-of-Non-negative-Functions-9",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Non-negative-Functions-9",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " For part (a) it follows from the definition of the Lebesgue integral for simple functions, that every simple function on has integral . Thus the supremum over all simple functions is also and .  For the proof of (b), we showed that for simple functions we have Taking the supremum over all simple functions with thus gives that   To prove (c), let denote the subset of such that and let . Then and   "
},
{
  "id": "thm-monotone-convergence",
  "level": "2",
  "url": "notes-week-07.html#thm-monotone-convergence",
  "type": "Theorem",
  "number": "75",
  "title": "Monotone Convergence Theorem.",
  "body": " Monotone Convergence Theorem   Let be a measure space and let be a monotonically increasing sequence of non-negative functions. If there is a finite for which for all , then converges to a finite-valued function almost everywhere, is integrable, and    "
},
{
  "id": "subsec-Integration-of-Non-negative-Functions-11",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Non-negative-Functions-11",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " We will first prove that converges almost everywhere, then we will show that and lastly we will show that   From the monotone convergent Theorem we know that for a given , converges provided that the sequence is bounded. Thus we want to show that the set of where is unbounded has measure zero. We know that each function is the limit of an increasing sequence of simple functions . Define a new sequence of simple functions by   Since  is an increasing sequence.  Since , the integrals are bounded. If is a point for which is unbounded, then for any , we can find an for which . Since converges to , we can find a for which .  Hence and thus the sequence is also unbounded.  By the Monotone Convergence Theorem for Simple Functions , the set of for which is unbounded has measure zero. Thus we are free to set on . We now have that is bounded almost everywhere and thus converges to a finite valued function almost everywhere, which is the first part of the proof.  For the next part suppose that and are non-negative measurable functions with for all . Then the set of simple functions that are less than or equal to is a subset of the set of simple functions that are less than or equal to and hence   If except possibly for where , then   We have shown that functional inequalities that hold almost everywhere imply integral inequalities that hold on all . Since almost everywhere, it follows that for all  and therefore,   To finish the proof of the Monotone Convergence Theorem we need to show the reverse inequality holds as well. To show the reverse inequality let be a non-negative simple function that is less than or equal to . That is, where the are disjoint sets with .  Choose with and define the sets   Since is an increasing sequence it follows that is an increasing sequence of sets with . Hence   Now choose . Then for each , we can find such that whenever ,   Let . Then for ,   Hence,   Since this is true for every and with , it follows that   Thus every simple function has an integral bounded above by for all sufficiently large. Hence and thus which completes the proof.  "
},
{
  "id": "thm-homogenous-integration",
  "level": "2",
  "url": "notes-week-07.html#thm-homogenous-integration",
  "type": "Theorem",
  "number": "76",
  "title": "Integration is Homogeneous.",
  "body": " Integration is Homogeneous   Suppose is a measure space and is a non-negative function. If , then    "
},
{
  "id": "subsec-Integration-of-Non-negative-Functions-13",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Non-negative-Functions-13",
  "type": "Proof",
  "number": "3",
  "title": "",
  "body": " Note that is a simple function with if and only if is a simple function with . Hence   "
},
{
  "id": "def-f-plus",
  "level": "2",
  "url": "notes-week-07.html#def-f-plus",
  "type": "Definition",
  "number": "77",
  "title": "<span class=\"process-math\">\\(f^+\\)<\/span> and <span class=\"process-math\">\\(f^-\\)<\/span>.",
  "body": " and   Suppose is a function. Define functions and by    "
},
{
  "id": "def-real-integral",
  "level": "2",
  "url": "notes-week-07.html#def-real-integral",
  "type": "Definition",
  "number": "78",
  "title": "Integral of a Real Valued Function.",
  "body": " Integral of a Real Valued Function   Suppose is a measure space and is an -measurable function such that at least one of or is finite. The integral of with respect to denoted by , is defined by    "
},
{
  "id": "thm-integration-is-homogeneous",
  "level": "2",
  "url": "notes-week-07.html#thm-integration-is-homogeneous",
  "type": "Theorem",
  "number": "79",
  "title": "Integration is Homogeneous.",
  "body": " Integration is Homogeneous   Suppose is a measure space and is a measurable function such that is defined. If , then    "
},
{
  "id": "subsec-Integration-of-Real-Valued-Functions-11",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Real-Valued-Functions-11",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " First suppose . Then and . Hence   If , then and thus and . Hence   "
},
{
  "id": "thm-additivity-integral",
  "level": "2",
  "url": "notes-week-07.html#thm-additivity-integral",
  "type": "Theorem",
  "number": "80",
  "title": "Additivity of Integration.",
  "body": " Additivity of Integration   Suppose is a measure space and are -measurable functions such that and . Then    "
},
{
  "id": "subsec-Integration-of-Real-Valued-Functions-13",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Real-Valued-Functions-13",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " We first observe that . Hence   Since both sides are non-negative functions we can integrate both sides of the above equation with respect to to get Rearranging the above equation gives   We note that the left side is not of the form since and . Thus we can simplify the above equation to obtain which is what we wanted to prove.  "
},
{
  "id": "thm-integration-order",
  "level": "2",
  "url": "notes-week-07.html#thm-integration-order",
  "type": "Theorem",
  "number": "81",
  "title": "Integration is Order Preserving.",
  "body": " Integration is Order Preserving   Suppose is a measure space and are -measurable functions such that and are defined. Suppose also that for all . Then .   "
},
{
  "id": "subsec-Integration-of-Real-Valued-Functions-15",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Real-Valued-Functions-15",
  "type": "Proof",
  "number": "3",
  "title": "",
  "body": " If or then the conclusion is always true, so suppose that and . It follows from additivity that   The right hand side is non-negative since for all . Moreover, the left hand side is finite due to our assumptions. Hence   "
},
{
  "id": "thm-absolute-value-integral",
  "level": "2",
  "url": "notes-week-07.html#thm-absolute-value-integral",
  "type": "Theorem",
  "number": "82",
  "title": "Absolute Value of Integral <span class=\"process-math\">\\(\\leq\\)<\/span> than Integral of Absolute Value.",
  "body": " Absolute Value of Integral than Integral of Absolute Value   Suppose is a function such that is defined. Then    "
},
{
  "id": "subsec-Integration-of-Real-Valued-Functions-17",
  "level": "2",
  "url": "notes-week-07.html#subsec-Integration-of-Real-Valued-Functions-17",
  "type": "Proof",
  "number": "4",
  "title": "",
  "body": " Because is defined and is an -measureable function at least one of and is finite. Hence, which is what we want to show.  "
},
{
  "id": "def-expected-value",
  "level": "2",
  "url": "notes-week-07.html#def-expected-value",
  "type": "Definition",
  "number": "83",
  "title": "Expected Value.",
  "body": " Expected Value   Suppose is a real valued random variable on a probability space . Then the expected value of is denoted is defined as the Lebesgue integral    "
},
{
  "id": "notes-week-08",
  "level": "1",
  ...
  "type": "Section",
  "number": "",
  "title": "Week 8: Limits of Integrals and Integrals of Limits (3B)",
  "body": " Week 8: Limits of Integrals and Integrals of Limits (3B)   Bounded Convergence Theorem  Suppose that is a sequence of integrable functions that converges to a limit function . An important question with many applications is when is it true that in other words, when are we permitted to exchange limit and integral?  In MTH 528 we saw that when the integral is the Riemann integral, if convergence is uniform, then we are permitted to exchange limit and integral. Can we do better with the Lebesgue Integral?  Example: Let be a (countable) list of the rational numbers and define functions by   Each is except at a finite number of points, so it is Riemann integrable with integral , but converges pointwise to Dirichlet's function which is not Riemann integrable. However, it is Lebesgue integrable with integral .  Example: Let   For each , is Lebesgue integrable with integral ; however, converges pointwise to , which has Lebesgue integral , so we can't exchange limit and integral. Notice that in the first example for all , but in the second example is unbounded.   Bounded Convergence Theorem   Suppose is a measure space with . Suppose is a sequence of -measurable functions from that converges pointwise on to a function . If there exists such that for all and , then      Bounding an Integral   Suppose is a measure space, is a subset in , and is a function such that is defined. Then      Let . Then    Now we can prove the Bounded Convergence Theorem   The main idea of the proof is that by Egorov's Theorem, every convergent sequence is almost uniformly convergent and if convergence is uniform then you can interchange limit and integral.  Let . Then by Egorov's Theorem there is a subset such that and converges uniformly to on . Then where the last line follows from applying the lemma.  Since converges uniformly to on , it follows that for large , . Hence for sufficiently large, and since is arbitrary this can be made as small as possible. Thus as .     Sets of Measure Zero in Integration  Example: Suppose that   Then converges pointwise to but both and are unbounded, so we can't use Bounded Convergence Theorem, or can we?  Note that is bounded almost everywhere; that is, it is only unbounded on a set of measure zero and since sets of measure zero are negligible with respect to integration we can still use BCT.  Define a new function by   Then converges pointwise to . Since is bounded by , we can use BCT to conclude that   Since it follows that   The ideas of this example can be universally applied to theorems about integrals. In most every case, the hypotheses of a theorem about integrals can be relaxed so that the hypotheses just hold almost everywhere; that is, outside a set of measure zero. The previous example shows that the BCT still holds if the sequence of functions is bounded almost everywhere on instead of needing to be bounded on all .    Dominated Convergence Theorem  The BCT says that we can interchange limit and integral if the sequence of functions is uniformly bounded and the measure of the space is finite. The MCT says that we can interchange limit and integral if the functions are non-negative and increasing. The Dominated Convergence Theorem does away with these restrictions. The only requirement for the DCT is that the sequence of functions can be bounded by another integrable function.   Dominated Convergence Theorem   Suppose is a measure space, is -measurable, and are -measurable functions from such that for almost every . If there exists an -measurable function such that for every and almost every , then     To prove the DCT we will need the following lemma.    Suppose is a measure space, is -measurable and . Then for every , there exists such that for every set in with .     Choose and let be a simple function such that and The existence of such a function is given by the definition of the Lebesgue integral for non-negative functions. Let and choose such that .  Now suppose and . Then    Now we can prove the Dominated Convergence Theorem.   Suppose satisfies the hypotheses of the theorem. If , then   Suppose that . Then by the first lemma, there exists such that for every set such that .  By Egorov's Theorem, there exists a set such that and the sequence converges uniformly to on . Thus for sufficiently large,   Since was arbitrarily chosen, this can be made as small as desired and hence      Riemann Integral and Lebesgue Integral  In MTH 528 we discussed Lebesgue's theorem, which stated that a function is Riemann integrable if and only if its discontinuities form a set of measure zero. We now have the tools to prove this result.   Lebesgue's Theorem   Suppose and is a bounded function. Then is Riemann integrable if and only if Furthermore if is Riemann integrable and denotes Lebesgue measure on then is Lebesgue measurable and      For this proof we will make use of the fact that the Riemann integral and the Darboux integral are equivalent so that we can use the definition of the Darboux integral. Choose and let be the partition that divides into subintervals of equal size. Let be the corresponding subintervals and note that each has length .  Now define This allows us to define the Lower and Upper sums in terms of integrals.   There is an issue with points on the boundary of two subintervals, so we will need to modify our definitions of and slightly. At points on a boundary of two subintervals, define to be the infimum over both subintervals and define to be the supremum over both subintervals. Since this only changes the values of and at a finite number of points, the lower and upper sums are still defined by the exact same integrals.  Taking the limit as is makes the partitions finer. Thus is an increasing sequence of functions and is a decreasing sequence of functions on . Define a lower function and upper function by   Since these sequences are bounded we can apply the BCT or DCT to interchange limit and integral. Hence   Likewise   The function is Riemann integrable if and only if . This means that is Riemann integrable if and only if   Since the only way that this integral is is for almost everywhere. In other words, the integral is if and only if   It follows from how and are defined that Thus is Riemann integrable if and only if its discontinuities form a set of measure zero.   Since every Riemann integrable function is Lebesgue integrable, we no longer need to distinguish between the two types of integrals. From now on, when we use the term integrable, we understand it to mean the Lebesgue integral. This also means that in many cases we don't need to use the definition of the Lebesgue integral when working with integrals. If we are integrating a function that is nicely integrable using the Riemann integral, we can use that instead, and likewise if we are integrating a function which has a nice antiderivative we can use the Fundamental Theorem of Calculus to integrate it.    Approximation by Nice Functions   Banach Space   A Banach space is a complete normed vector space.    Informally a Banach space is a set of functions which is closed under addition, scalar multiplication, and limits, and in which there is a well defined notion of distance. Banach spaces are widely studied in functional analysis. The set of Lebesgue integrable functions on a measure space is almost, but not quite, a Banach space. We will prove some of the properties of this set.   ,   Suppose is a measure space. If is -measurable, then the norm of is denoted by and defined by   The Lebesgue space is defined by     Note: Technically is a semi-norm, not a norm, but for convenience we will call it a norm.  Example: Let denote Lebesgue measure on . Then is the set of functions whose Lebesgue integral over the entire real line is finite. This is typically denoted as .  Let denote the counting measure on and let be a sequence of real numbers. (Recall that a sequence of numbers can be thought of as a function on .) Then . In this case is denoted by . In other words is the set of sequences of real numbers such that .   Properties of the Norm   Suppose is a measure space and . Then    .     if and only if for almost every .     for every .     . (Triangle Inequality)        Because , it follows that Thus (a) is proven.  Moreover if and only if if and only if for almost every . Thus (b) is proved.  To prove (c) note that   Finally we prove (d).    Next we will show that every function in can be approximated in by simple functions.   Approximation by Simple Functions   Suppose is a measure and . Then for every there exists a simple function such that     A corollary of this theorem is that given , there exists a sequence of simple functions that converges to in the norm; that is,    The idea of the proof is that we can approximate and by simple functions and we can use these to approximate . Let . Then there exists simple functions and such that , , and   Now let . Then is a simple function and as desired.   It turns out that we can do even better than approximate functions by simple functions; we can approximate them by step functions.   Step Function   A step function is a function of the form where , , \\ldots are intervals of and are nonzero real numbers.    Recall that every simple function can be written in the form where each for real number .  The difference between a step function and a simple function is that in a simple function the sets do not have to be intervals, but they do in a step function. Every step function is a simple function, but not every simple function is a step function.   Approximation by Step Functions   Suppose . Then for every , there exists a step function such that .     The idea of the proof is to first approximate by simple functions and then approximate the simple functions by step functions. Let . Then there exists Lebesgue measurable sets , , \\ldots of and nonzero numbers , , \\ldots such that for all and   Now because every measurable set is almost an open set, for each there exists an open set such that and . Moreover, each open subset of can be expressed as a countable union of disjoint open intervals. Thus for each there exists a set that is a finite union of disjoint open intervals such that and .  Hence . Observe that   Hence   Thus it follows that   Since can be made arbitrarily small, the last line can be made as small as we desire. Thus we can approximate by step functions.   Finally we approximate functions by continuous functions.   Approximation by Continuous Functions   Suppose . Then for every there exists a continuous function such that and is a bounded set.     The idea of the proof is that we can approximate step functions by continuous functions by connecting the pieces with lines.   A graph of a step function.   Graph of a step function with three yellow-orange horizontal segments: a short segment along the horizontal axis near the origin, followed by a high constant segment, and then a lower constant segment farther to the right. The coordinate axes are black.     A continuous piecewise-linear approximation to the step function.   Graph comparing a yellow-orange three-level step function with a blue continuous piecewise-linear approximation. The blue graph follows each horizontal portion of the step function and uses steep sloping segments to transition continuously from the horizontal axis to the highest level and then down to the intermediate level. The axes are unlabeled and black.    For every , , and it follows from the properties of the norm that   Since we can approximate functions by step functions, we can find numbers and to make as small as we want.  As in the picture, we can construct continuous functions so that is as small as we want. Thus letting gives us a continuous function that can be made arbitrarily close to as desired.    "
},
{
  "id": "thm-BCT",
  "level": "2",
  "url": "notes-week-08.html#thm-BCT",
  "type": "Theorem",
  "number": "84",
  "title": "Bounded Convergence Theorem.",
  "body": " Bounded Convergence Theorem   Suppose is a measure space with . Suppose is a sequence of -measurable functions from that converges pointwise on to a function . If there exists such that for all and , then    "
},
{
  "id": "lem-integral-bound",
  "level": "2",
  "url": "notes-week-08.html#lem-integral-bound",
  "type": "Lemma",
  "number": "85",
  "title": "Bounding an Integral.",
  "body": " Bounding an Integral   Suppose is a measure space, is a subset in , and is a function such that is defined. Then    "
},
{
  "id": "subsec-Bounded-Convergence-Theroem-10",
  "level": "2",
  "url": "notes-week-08.html#subsec-Bounded-Convergence-Theroem-10",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " Let . Then   "
...
{
  "id": "subsec-Bounded-Convergence-Theroem-12",
  "level": "2",
  "url": "notes-week-08.html#subsec-Bounded-Convergence-Theroem-12",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " The main idea of the proof is that by Egorov's Theorem, every convergent sequence is almost uniformly convergent and if convergence is uniform then you can interchange limit and integral.  Let . Then by Egorov's Theorem there is a subset such that and converges uniformly to on . Then where the last line follows from applying the lemma.  Since converges uniformly to on , it follows that for large , . Hence for sufficiently large, and since is arbitrary this can be made as small as possible. Thus as .  "
},
{
  "id": "thm-DCT",
  "level": "2",
  "url": "notes-week-08.html#thm-DCT",
  "type": "Theorem",
  "number": "86",
  "title": "Dominated Convergence Theorem.",
  "body": " Dominated Convergence Theorem   Suppose is a measure space, is -measurable, and are -measurable functions from such that for almost every . If there exists an -measurable function such that for every and almost every , then    "
},
{
  "id": "lem-integrals-on-small-sets-are-small",
  "level": "2",
  "url": "notes-week-08.html#lem-integrals-on-small-sets-are-small",
  "type": "Lemma",
  "number": "87",
  "title": "",
  "body": "  Suppose is a measure space, is -measurable and . Then for every , there exists such that for every set in with .   "
},
{
  "id": "subsec-Dominated-Convergence-Theorem-6",
  "level": "2",
  "url": "notes-week-08.html#subsec-Dominated-Convergence-Theorem-6",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " Choose and let be a simple function such that and The existence of such a function is given by the definition of the Lebesgue integral for non-negative functions. Let and choose such that .  Now suppose and . Then   "
},
{
  "id": "subsec-Dominated-Convergence-Theorem-8",
  "level": "2",
  "url": "notes-week-08.html#subsec-Dominated-Convergence-Theorem-8",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " Suppose satisfies the hypotheses of the theorem. If , then   Suppose that . Then by the first lemma, there exists such that for every set such that .  By Egorov's Theorem, there exists a set such that and the sequence converges uniformly to on . Thus for sufficiently large,   Since was arbitrarily chosen, this can be made as small as desired and hence   "
},
{
  "id": "thm-Lebesgue",
  "level": "2",
  "url": "notes-week-08.html#thm-Lebesgue",
  "type": "Theorem",
  "number": "88",
  "title": "Lebesgue’s Theorem.",
  "body": " Lebesgue's Theorem   Suppose and is a bounded function. Then is Riemann integrable if and only if Furthermore if is Riemann integrable and denotes Lebesgue measure on then is Lebesgue measurable and    "
},
{
  "id": "subsec-Riemann-Integral-and-Lebesgue-Integral-4",
  "level": "2",
  "url": "notes-week-08.html#subsec-Riemann-Integral-and-Lebesgue-Integral-4",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " For this proof we will make use of the fact that the Riemann integral and the Darboux integral are equivalent so that we can use the definition of the Darboux integral. Choose and let be the partition that divides into subintervals of equal size. Let be the corresponding subintervals and note that each has length .  Now define This allows us to define the Lower and Upper sums in terms of integrals.   There is an issue with points on the boundary of two subintervals, so we will need to modify our definitions of and slightly. At points on a boundary of two subintervals, define to be the infimum over both subintervals and define to be the supremum over both subintervals. Since this only changes the values of and at a finite number of points, the lower and upper sums are still defined by the exact same integrals.  Taking the limit as is makes the partitions finer. Thus is an increasing sequence of functions and is a decreasing sequence of functions on . Define a lower function and upper function by   Since these sequences are bounded we can apply the BCT or DCT to interchange limit and integral. Hence   Likewise   The function is Riemann integrable if and only if . This means that is Riemann integrable if and only if   Since the only way that this integral is is for almost everywhere. In other words, the integral is if and only if   It follows from how and are defined that Thus is Riemann integrable if and only if its discontinuities form a set of measure zero.  "
},
{
  "id": "def-Banach-space",
  "level": "2",
  "url": "notes-week-08.html#def-Banach-space",
  "type": "Definition",
  "number": "89",
  "title": "Banach Space.",
  "body": " Banach Space   A Banach space is a complete normed vector space.   "
},
{
  "id": "def-L-1",
  "level": "2",
  "url": "notes-week-08.html#def-L-1",
  "type": "Definition",
  "number": "90",
  "title": "<span class=\"process-math\">\\(||f||_1\\text{,}\\)<\/span> <span class=\"process-math\">\\(\\mathcal{L}^1(\\mu)\\)<\/span>.",
  "body": " ,   Suppose is a measure space. If is -measurable, then the norm of is denoted by and defined by   The Lebesgue space is defined by    "
},
{
  "id": "thm-L-1-properties",
  "level": "2",
  "url": "notes-week-08.html#thm-L-1-properties",
  "type": "Theorem",
  "number": "91",
  "title": "Properties of the <span class=\"process-math\">\\(\\mathcal{L}^1\\)<\/span> Norm.",
  "body": " Properties of the Norm   Suppose is a measure space and . Then    .     if and only if for almost every .     for every .     . (Triangle Inequality)      "
},
{
  "id": "subsec-Approximation-by-Nice-Functions-9",
  "level": "2",
  "url": "notes-week-08.html#subsec-Approximation-by-Nice-Functions-9",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " Because , it follows that Thus (a) is proven.  Moreover if and only if if and only if for almost every . Thus (b) is proved.  To prove (c) note that   Finally we prove (d).   "
},
{
  "id": "thm-approximation-simple-function",
  "level": "2",
  "url": "notes-week-08.html#thm-approximation-simple-function",
  "type": "Theorem",
  "number": "92",
  "title": "Approximation by Simple Functions.",
  "body": " Approximation by Simple Functions   Suppose is a measure and . Then for every there exists a simple function such that    "
},
{
  "id": "subsec-Approximation-by-Nice-Functions-13",
  "level": "2",
  "url": "notes-week-08.html#subsec-Approximation-by-Nice-Functions-13",
  "type": "Proof",
  "number": "2",
  "title": "",
  "body": " The idea of the proof is that we can approximate and by simple functions and we can use these to approximate . Let . Then there exists simple functions and such that , , and   Now let . Then is a simple function and as desired.  "
},
{
  "id": "def-step-function",
  "level": "2",
  "url": "notes-week-08.html#def-step-function",
  "type": "Definition",
  "number": "93",
  "title": "Step Function.",
  "body": " Step Function   A step function is a function of the form where , , \\ldots are intervals of and are nonzero real numbers.   "
},
{
  "id": "thm-approximation-step-functions",
  "level": "2",
  "url": "notes-week-08.html#thm-approximation-step-functions",
  "type": "Theorem",
  "number": "94",
  "title": "Approximation by Step Functions.",
  "body": " Approximation by Step Functions   Suppose . Then for every , there exists a step function such that .   "
},
{
  "id": "subsec-Approximation-by-Nice-Functions-19",
  "level": "2",
  "url": "notes-week-08.html#subsec-Approximation-by-Nice-Functions-19",
  "type": "Proof",
  "number": "3",
  "title": "",
  "body": " The idea of the proof is to first approximate by simple functions and then approximate the simple functions by step functions. Let . Then there exists Lebesgue measurable sets , , \\ldots of and nonzero numbers , , \\ldots such that for all and   Now because every measurable set is almost an open set, for each there exists an open set such that and . Moreover, each open subset of can be expressed as a countable union of disjoint open intervals. Thus for each there exists a set that is a finite union of disjoint open intervals such that and .  Hence . Observe that   Hence   Thus it follows that   Since can be made arbitrarily small, the last line can be made as small as we desire. Thus we can approximate by step functions.  "
},
{
  "id": "thm-approximation-continuous-function",
  "level": "2",
  "url": "notes-week-08.html#thm-approximation-continuous-function",
  "type": "Theorem",
  "number": "95",
  "title": "Approximation by Continuous Functions.",
  "body": " Approximation by Continuous Functions   Suppose . Then for every there exists a continuous function such that and is a bounded set.   "
},
{
  "id": "subsec-Approximation-by-Nice-Functions-22",
  "level": "2",
  "url": "notes-week-08.html#subsec-Approximation-by-Nice-Functions-22",
  "type": "Proof",
  "number": "4",
  "title": "",
...
},
{
  ...
  "type": "Worksheet",
  "number": "",
  "title": "Homework 5 (Due October 1)",
  "body": " Homework 5 (Due October 1)    Instructions: Complete all the exercises below and submit your work by the due date.      Let denote the Borel -algebra on and let be defined by Prove that is a measure on .      Let and suppose that is a measure space. Either prove that is translation invariant; that is for , , where , or find a counterexample to this claim.      Let and suppose that is a measure space. Is it always true that for , , where ? Provide a proof or a counterexample.      Give an example of a -algebra and measures and such that for all , but .      Suppose that and are measures on a measurable space . Define a set function by for . Show that is a measure on      "
},
{
  "id": "homework-6-3-1",
  "level": "2",
  ...
  "number": "1",
  "title": "",
  "body": "  Let denote the Borel -algebra on and let be defined by Prove that is a measure on .   "
},
{
  "id": "homework-6-3-2",
  "level": "2",
  ...
  "number": "2",
  "title": "",
  "body": "  Let and suppose that is a measure space. Either prove that is translation invariant; that is for , , where , or find a counterexample to this claim.   "
},
{
  "id": "homework-6-3-3",
  "level": "2",
  ...
  "number": "3",
  "title": "",
  "body": "  Let and suppose that is a measure space. Is it always true that for , , where ? Provide a proof or a counterexample.   "
},
{
  "id": "homework-6-3-4",
  "level": "2",
  ...
  "number": "4",
  "title": "",
  "body": "  Give an example of a -algebra and measures and such that for all , but .   "
},
{
  "id": "homework-6-3-5",
  "level": "2",
